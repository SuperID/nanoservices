...
exports.compileLogFormat = function (template) {
  ...
  const lines = Object.keys(vars).map(n => {
    ...
    if (n === 'service') return `const service = ctx._service && ctx._service.name || null;`;
    if (n === 'uptime') return `const uptime = Date.now() - ctx.startTime.getTime();`;
    ...
    if (n === 'isotime') return `const isotime = new Date().toISOString();`;
