...
class Context {
  ...
  constructor(options) {
    ...
    this.writeJSON('call', this.params);
  }
  ...
  callback(err, ret) {
    setImmediate(() => {
      ...
      if (err) {
        this.writeJSON('error', formatErrorObject(err));
      } else {
        this.writeJSON('result', ret);
      }
   ...
   */
  writeJSON(type, data) {
    this.write(type, utils.jsonStringify(data));
  }
   ...
   */
  debug() {
    this.writeJSON('debug', utils.format.apply(null, arguments));
  }
   ...
   */
  log() {
    this.writeJSON('log', utils.format.apply(null, arguments));
  }
  ...
  call(name, params, callback) {
    if (typeof callback === 'function') {
      this._call(name, params, callback);
    } else {
      return utils.newPromise((resolve, reject) => {
        this._call(name, params, (err, ret) => err ? reject(err) : resolve(ret));
      });
    }
  }

  _call(name, params, callback) {
    // 查询服务
  ...
  transfer(name, params) {
    this.call(name, params, (err, ret) => this.callback(err, ret));
  }
   ...
   */
  transferSeries(list) {
    this.series(list, (err, ret) => this.callback(err, ret));
  }
  ...
  series(list, callback) {
    if (typeof callback === 'function') {
      this._series(list, callback);
    } else {
      return utils.newPromise((resolve, reject) => {
        this._series(list, (err, ret) => err ? reject(err) : resolve(ret));
      });
    }
  }

  _series(list, callback) {
