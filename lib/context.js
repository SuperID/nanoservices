...
class Context {
  ...
  callback(err, ret) {
    ...
    this.log('[callback] spent=%s, err=%s, ret=%j', this.spent, err, ret);
   ...
   */
  debug(...args) {
    this._writeLog && this._writeLog(`${utils.date('Y-m-d H:i:s')} ${this.requestId} [debug] ${utils.format(...args)}`);
  }
   ...
   */
  log(...args) {
    this._writeLog && this._writeLog(`${utils.date('Y-m-d H:i:s')} ${this.requestId} [log] ${utils.format(...args)}`);
  }
