...
class Context {
  ...
  constructor(options) {
    ...
    if (options.params) {
      const paramsType = typeof options.params;
    ...
    // 复制并冻结参数对象
    this.params = Object.freeze(Object.assign({}, options.params || {}));
    // 回调函数
   ...
   */
  write(type, text) {
    this._writeLog && this._writeLog(`${utils.date('Y-m-d H:i:s')} [${this.requestId}] [${type}] ${text}`);
  }
